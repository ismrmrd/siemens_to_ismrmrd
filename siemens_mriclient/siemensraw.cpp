...
int SiemensRawData::ReadRawFile(char* filename)
{
	...
	for (unsigned int i = 0; i < number_of_parameter_buffers; i++) {
		...
		m_parameter_buffers[std::string(buffer_name)] = std::string(tmp_buffer,buffer_length-2);//-2 because the last two character are ^@
...
int SiemensRawData::ParseMeasYaps()
{
	...
	m_base_parameters.dwell_time_us =  atof(m_meas_yaps["sRXSPEC.alDwellTime[0]"].c_str())/1000.0f;
